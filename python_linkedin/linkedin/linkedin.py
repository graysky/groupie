...
class Group(object):
    ...
    def create(node):
        ...
        """
        document = minidom.parseString(node)
        children = document.getElementsByTagName("group")
        result = []
        for child in children:
            group = Group()
            group.id = group._get_child(child, "id")
            group.name = group._get_child(child, "name")
            result.append(group)
...
class LinkedIn(object):
    ...
    def get_groups(self, member_id = None):
        """
        Fetches the groups for a user whose id is the given member_id 
        @Returns: a list of Group instances or an empty list if there is no connection.
        ...
        * http://api.linkedin.com/v1/people/~/group-memberships:(group:(id,name,counts-by-category))?membership-state=member
        ...
        raw_url = "/v1/people/~/group-memberships"
        
        try:
            response = self._do_normal_query(raw_url)
            document = minidom.parseString(response)
            memberships = document.getElementsByTagName("group-membership")
            
            result = []
            for membership in memberships:
                group = Group.create(membership.toxml())
